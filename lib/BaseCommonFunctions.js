...
function CommonFunctions () {
  ...
  this.getAccessibilityServiceClassName = function () {
    if (this.getAutoJsPackage() === 'org.autojs.autojs6') {
      return 'org.autojs.autojs.core.accessibility.AccessibilityServiceUsher'
    }
        ...
        do {
          ...
          if (!screen) {
            ...
            if (errorLimit > errorCount + 1) {
              ...
              // Android 14+ 的 MediaProjection token 失效后，切换多任务无法恢复截图，
              // 反而会打断正在执行的应用。保留后续的统一权限重建流程。
              _logUtils.debugInfo('获取截图失败，等待统一截图权限重建，不切换多任务界面')
            }
  ...
  this.recordTimedTask = function (task) {
    if (!task) {
      _logUtils.warnInfo('定时任务创建失败，未记录自动启动任务')
      return
    }
    // AutoJs6 的 TimedTask 是 Java 对象，直接 JSON 序列化会写入 null。
    // 仅保存清理任务所需的字段，兼容旧版 m* 字段和 AutoJs6 getter。
    let taskInfo = {
      mId: task.mId || task.id || (typeof task.getId === 'function' && task.getId()),
      mScriptPath: task.mScriptPath || task.scriptPath || (typeof task.getScriptPath === 'function' && task.getScriptPath()),
      mMillis: task.mMillis || task.millis || (typeof task.getMillis === 'function' && task.getMillis())
    }
    if (!taskInfo.mId || !taskInfo.mScriptPath) {
      _logUtils.warnInfo(['定时任务信息不完整，未记录自动启动任务：{}', JSON.stringify(taskInfo)])
      return
    }
  ...
  this.cancelAllTimedTasks = function (forceMain) {
    ...
    if (array && array.length > 0) {
      ...
      array = array.filter(task => {
        if (!task || !task.mScriptPath) {
          _logUtils.warnInfo('忽略无效的自动启动任务缓存')
          return false
        }
    ...
    try {
      ...
      actionThread = threads.start(function () {
        try {
          ...
          let isInterrupted = context.getPackageName() === 'org.autojs.autojs6'
            ? e.javaException instanceof org.autojs.autojs.runtime.exception.ScriptInterruptedException
            : e.javaException instanceof com.stardust.autojs.runtime.exception.ScriptInterruptedException
          if (isInterrupted) {
            return
          }
  ...
  */
  ...
  this.reduceConsoleLogs = function () {
    if (!_config.is_pro) {
      ...
      let consoleImpl = isAutoJs6
        ? org.autojs.autojs.AutoJs.getInstance().getGlobalConsole()
        : org.autojs.autojs.autojs.AutoJs.getInstance().getGlobalConsole()
      let logList = isAutoJs6 ? consoleImpl.getLogEntries() : consoleImpl.getAllLogs()
  ...
  this.convertImageFromSingleChannel = function (singleChannelImage) {
    let img = context.getPackageName() === 'org.autojs.autojs6'
      ? images.cvtColor(singleChannelImage, 'GRAY2BGRA')
      : com.stardust.autojs.core.image.ImageWrapper.ofBitmap(singleChannelImage.getBitmap())
