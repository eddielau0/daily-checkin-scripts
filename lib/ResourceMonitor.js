...
module.exports = function (__runtime__, scope) {
  if (typeof scope.resourceMonitor === 'undefined' || scope.resourceMonitor === null) {
    ...
    // AutoJs6 6.7 已移除旧版内部模块 __images__.js，图片 API 会直接注入
    // 全局 images 对象。保留旧版加载方式，只在 AutoJs6 下复用现有对象。
    let _o_images = isAutoJs6 ? scope.images : require('__images__.js')(__runtime__, scope)
    ...
    ResourceMonitor.prototype.init = function () {
      ...
      // AutoJs6 的 images 是增强后的普通脚本对象，不是带 prototype 的
      // 构造函数，因此仅建立用于覆盖方法的空原型。
      M_Images.prototype = isAutoJs6 ? {} : Object.create(_o_images.prototype)
      ...
      // AutoJs6 的 images 还包含事件发射器等原型方法。requestScreenCapture
      // 会直接读取 images.emit，使用原对象作为原型可保留这些运行时方法。
      let newImages = isAutoJs6 ? Object.create(_o_images) : {}
