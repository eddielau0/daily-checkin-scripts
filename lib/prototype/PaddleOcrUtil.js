...
function PaddleOcrUtil () {
  ...
  this.autoJs6Support = typeof ocr !== 'undefined' && typeof ocr.paddle !== 'undefined'
...
PaddleOcrUtil.prototype.init = function () {
  if (this.autoJs6Support) {
    LogUtils.debugInfo(['使用AutoJs6内置Paddle OCR接口'])
    this.ocr = ocr.paddle
...
PaddleOcrUtil.prototype.initWithCustomizeModel = function (options) {
  options = options || {}
  if (this.autoJs6Support) {
    LogUtils.warnInfo(['AutoJs6 6.7的Paddle OCR已插件化，不支持脚本内加载自定义模型'])
    return false
  }
  if (typeof $ocr !== 'undefined') {
    this.internalSupport = true
...
PaddleOcrUtil.prototype.recognize = function (img, region) {
  if (this.internalSupport) {
    ...
    return (this.autoJs6Support ? this.ocr : $ocr).recognizeText(img, options).join('\n')
  }
  if (!this.initialized) {
    this.init()
  }
...
PaddleOcrUtil.prototype.recognizeWithBounds = function (img, region, regex, showOcrDetail) {
  if (this.internalSupport) {
    let options = this.autoJs6Support && region == null ? {} : { region: region }
    let result = (this.autoJs6Support ? this.ocr : $ocr).detect(img, options).filter(r => r.confidence > 0.5)
    if (regex) {
