...
module.exports = (function (_runtime_, scope) {
  ...
  // AutoJs6 6.7 已提供公开的 tasks API，内部的 TimedTaskManager 是 Kotlin
  // object，不再提供旧版 Auto.js 使用的 getInstance() 入口。
  if (context.getPackageName() === 'org.autojs.autojs6' && typeof tasks !== 'undefined') {
    let autoJs6Timers = Object.create(_runtime_.timers)
    let taskMethods = [
      'addDailyTask', 'addWeeklyTask', 'addDisposableTask', 'addIntentTask',
      'getTimedTask', 'getIntentTask', 'removeTimedTask', 'removeIntentTask',
      'queryTimedTasks', 'queryIntentTasks'
    ]
    taskMethods.forEach(method => {
      autoJs6Timers[method] = function () {
        return tasks[method].apply(tasks, arguments)
      }
    })
    return autoJs6Timers
  }
